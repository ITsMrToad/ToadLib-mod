...
public class ToadEntityUtils {
    ...
    public static void villagerCraftItem(Villager villager, Item result, Item ingredient, int maxCount) {
        SimpleContainer simpleInventory = villager.getInventory();

        if (simpleInventory.countItem(result) <= maxCount) {
            int i = simpleInventory.countItem(ingredient);
            int j = 1;
            int k = 1;
            ...
            if (l != 0) {
                int i1 = l * k;
                simpleInventory.removeItemType(ingredient, i1);
                ItemStack itemstack = simpleInventory.addItem(new ItemStack(result, l));

                if (!itemstack.isEmpty()) {
                    villager.spawnAtLocation((ServerLevel) villager.level(), itemstack, 0.5F);
                }
            }
        }
    }
