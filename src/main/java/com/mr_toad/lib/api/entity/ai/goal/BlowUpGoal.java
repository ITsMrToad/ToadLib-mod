...
public class BlowUpGoal<M extends PathfinderMob & BlowUpDataContainer> extends Goal {
    ...
    public void tick() {
        ...
        } else if (this.mob.distanceTo(this.target) > 49.0) {
            this.mob.setSwellDir(-1);
        } else if (!this.shouldExplodeIfNotSee && !this.mob.hasLineOfSight(this.target)) {
            this.mob.setSwellDir(-1);
        } else {
            this.mob.setSwellDir(1);
        }
        ...
        if (this.mob.isAlive()) {
            ...
            if (this.invulnerableIfIgnited) {
                this.mob.isInvulnerableTo((ServerLevel) this.mob.level(), this.mob.damageSources().mobAttack(this.target));
            }
    ...
    public boolean canExplode() {
        return this.mob.getSwellDir() > 0 || this.mob.getTarget() != null && this.mob.distanceTo(this.mob.getTarget()) < this.distToSqr;
    }
    ...
    public void loadBlowUpData(CompoundTag nbt) {
        if (nbt.contains("Fuse")) {
            this.maxSwell = nbt.getShortOr("Fuse", (short) 5);
        }
        if (nbt.contains("ExplosionRadius")) {
            this.explosionRadius = nbt.getFloatOr("ExplosionRadius", 3.0F);
        }
        if (nbt.getBooleanOr("Ignited", false)) {
            this.mob.ignite();
        }
    }
