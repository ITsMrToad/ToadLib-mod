...
public class ToadConfigsScreen<S extends Screen> extends ParentableToadLibScreen<S> {
    ...
    @Override
    protected void init() {
        this.configSelectionList = this.addRenderableWidget(new ConfigSelectionList(this.getMinecraft(), this.width, this.height, 30, this.height - 20, this));
        this.refreshButton = this.addRenderableWidget(ToadClientUtils.createRefreshButton(40, 10, 20, 20, b -> this.getMinecraft().reloadResourcePacks()));
        this.refreshButton.setTooltip(Tooltip.create(REFRESH));
        this.folderButton = this.addRenderableWidget(new LinkButton(73, 7, 20, 20, LinkButton.DefaultType.DATA_FOLDER, FMLPaths.CONFIGDIR.get()));
