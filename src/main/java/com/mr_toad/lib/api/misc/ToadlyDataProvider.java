...
public class ToadlyDataProvider {
    ...
    public abstract static class AbstractToadlyItemModelProvider extends ItemModelProvider {
        ...
        public void blockBasedModel(Item item, String suffix, String modid) {
            withExistingParent(ToadOtherUtils.itemName(item).orElse(null), ToadOtherUtils.resourceBlock(ToadOtherUtils.itemName(item).orElse(null) + suffix, modid));
        }

        public void itemGeneratedModel(Item item, ResourceLocation texture) {
            withExistingParent(ToadOtherUtils.itemName(item).orElse(null), "item/generated").texture("layer0", texture);
        }
