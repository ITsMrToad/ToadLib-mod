...
public class KeyframeChannel {
    ...
    public float getValue() {
        if (this.getKeyframes().isEmpty()) {
            ...
            if (this.getKeyframes().size() == 1 || this.currentTick <= this.getKeyframes().getFirst().timestamp()) {
                return this.getKeyframes().getFirst().value();
    ...
    public float getDuration() {
        if (this.getKeyframes().size() == 1) {
            ...
            return (this.getKeyframes().get(this.getKeyframes().size() - 1).timestamp() - (this.isRunning() ? this.startTick : this.getKeyframes().getFirst().timestamp())) / 20F;
        }
    }
